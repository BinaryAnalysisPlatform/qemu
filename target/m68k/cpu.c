...
static void m68k_restore_state_to_opc(CPUState *cs, const TranslationBlock *tb,
                                      const uint64_t *data) {
  M68kCPU *cpu = M68K_CPU(cs);
  int cc_op = data[1];
...
static void m68k_cpu_reset_hold(Object *obj, ResetType type) {
   ...
   */
  set_floatx80_behaviour(
      floatx80_default_inf_int_bit_is_zero | floatx80_pseudo_inf_valid |
          floatx80_pseudo_nan_valid | floatx80_unnormal_valid |
...
static const VMStateDescription vmstate_freg_tmp = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT64(tmp_mant, m68k_FPReg_tmp),
...
const VMStateDescription vmmstate_fpu = {
    ...
    .fields = (const VMStateField[]){
        VMSTATE_UINT32(env.fpcr, M68kCPU), VMSTATE_UINT32(env.fpsr, M68kCPU),
...
const VMStateDescription vmstate_cf_spregs = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT64_ARRAY(env.macc, M68kCPU, 4),
...
const VMStateDescription vmstate_68040_mmu = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_UINT32(env.mmu.mmusr, M68kCPU), VMSTATE_END_OF_LIST()}};
...
const VMStateDescription vmstate_68040_spregs = {
    ...
    .fields = (const VMStateField[]){
        VMSTATE_UINT32(env.vbr, M68kCPU), VMSTATE_UINT32(env.cacr, M68kCPU),
        VMSTATE_UINT32(env.sfc, M68kCPU), VMSTATE_UINT32(env.dfc, M68kCPU),
        VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_m68k_cpu = {
    ...
    .fields =
        (const VMStateField[]){
            ...
            VMSTATE_UINT32(env.pc, M68kCPU), VMSTATE_UINT32(env.sr, M68kCPU),
            ...
            VMSTATE_INT32(env.pending_level, M68kCPU), VMSTATE_END_OF_LIST()},
    .subsections =
        (const VMStateDescription *const[]){&vmmstate_fpu, &vmstate_cf_spregs,
                                            &vmstate_68040_mmu,
                                            &vmstate_68040_spregs, NULL},
};
...
static void m68k_cpu_class_init(ObjectClass *c, void *data) {
  ...
  device_class_set_parent_realize(dc, m68k_cpu_realizefn, &mcc->parent_realize);
