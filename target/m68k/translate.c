...
#define DEFO32(name, offset)                                                   \
  QREG_##name =                                                                \
      tcg_global_mem_new_i32(tcg_env, offsetof(CPUM68KState, offset), #name);
#define DEFO64(name, offset)                                                   \
  QREG_##name =                                                                \
      tcg_global_mem_new_i64(tcg_env, offsetof(CPUM68KState, offset), #name);
#include "qregs.h.inc"
#undef DEFO32
#undef DEFO64

  cpu_halted = tcg_global_mem_new_i32(
      tcg_env, -offsetof(M68kCPU, env) + offsetof(CPUState, halted), "HALTED");
  cpu_exception_index = tcg_global_mem_new_i32(
      tcg_env, -offsetof(M68kCPU, env) + offsetof(CPUState, exception_index),
      "EXCEPTION");
  ...
  for (i = 0; i < 8; i++) {
    sprintf(p, "D%d", i);
    cpu_dregs[i] =
        tcg_global_mem_new(tcg_env, offsetof(CPUM68KState, dregs[i]), p);
    p += 3;
    sprintf(p, "A%d", i);
    cpu_aregs[i] =
        tcg_global_mem_new(tcg_env, offsetof(CPUM68KState, aregs[i]), p);
    p += 3;
  }
  for (i = 0; i < 4; i++) {
    sprintf(p, "ACC%d", i);
    cpu_macc[i] =
        tcg_global_mem_new_i64(tcg_env, offsetof(CPUM68KState, macc[i]), p);
    p += 5;
  }
...
static void delay_set_areg(DisasContext *s, unsigned regno, TCGv val,
                           bool give_temp) {
  if (s->writeback_mask & (1 << regno)) {
    if (give_temp) {
...
#define SFC_INDEX(s)                                                           \
  ((s->base.tb->flags & TB_FLAGS_SFC_S) ? MMU_KERNEL_IDX : MMU_USER_IDX)
#define DFC_INDEX(s)                                                           \
  ((s->base.tb->flags & TB_FLAGS_DFC_S) ? MMU_KERNEL_IDX : MMU_USER_IDX)
#endif
...
#else
#define DISAS_INSN(name)                                                       \
  static void disas_##name(CPUM68KState *env, DisasContext *s, uint16_t insn)
#endif
...
static inline TCGv gen_load(DisasContext *s, int opsize, TCGv addr, int sign,
  ...
  case OS_WORD:
  case OS_LONG:
    tcg_gen_qemu_ld_tl(tmp, addr, index, opsize | (sign ? MO_SIGN : 0) | MO_TE);
    break;
  default:
...
typedef enum { EA_STORE, EA_LOADU, EA_LOADS } ea_what;
...
static void gen_update_cc_cmp(DisasContext *s, TCGv dest, TCGv src,
                              int opsize) {
 ...
 */
static TCGv gen_lea_mode(CPUM68KState *env, DisasContext *s, int mode, int reg0,
                         int opsize) {
  TCGv reg;
  TCGv tmp;
  uint16_t ext;
...
static void gen_ldst_fp(DisasContext *s, int opsize, TCGv addr, TCGv_ptr fp,
                        ea_what what, int index) {
  if (what == EA_STORE) {
...
#define DEST_EA(env, insn, opsize, val, addrp)                                 \
  do {                                                                         \
    TCGv ea_result =                                                           \
        gen_ea(env, s, insn, opsize, val, addrp, EA_STORE, IS_USER(s));        \
...
DISAS_INSN(undef_mac) { gen_exception(s, s->base.pc_next, EXCP_LINEA); }

DISAS_INSN(undef_fpu) { gen_exception(s, s->base.pc_next, EXCP_LINEF); }
...
DISAS_INSN(abcd_mem) {
  ...
  src = gen_ea_mode(env, s, 4, REG(insn, 0), OS_BYTE, NULL_QREG, NULL, EA_LOADU,
                    IS_USER(s));
  dest = gen_ea_mode(env, s, 4, REG(insn, 9), OS_BYTE, NULL_QREG, &addr,
                     EA_LOADU, IS_USER(s));
    ...
    if (mode == 4) {
      ...
      for (i = 15; i >= 0; i--) {
        if ((mask << i) & 0x8000) {
          ...
          if (reg0 + 8 == i && m68k_feature(s->env, M68K_FEATURE_EXT_FULL)) {
            /*
...
DISAS_INSN(cas) {
  ...
  load = tcg_temp_new();
  tcg_gen_atomic_cmpxchg_i32(load, addr, cmp, DREG(ext, 6), IS_USER(s), opc);
...
DISAS_INSN(cas2w) {
  ...
  if (tb_cflags(s->base.tb) & CF_PARALLEL) {
    ...
    TCGv regs = tcg_constant_i32(REG(ext2, 6) | (REG(ext1, 6) << 3) |
                                 (REG(ext2, 0) << 6) | (REG(ext1, 0) << 9));
...
DISAS_INSN(cas2l) {
  ...
  regs = tcg_constant_i32(REG(ext2, 6) | (REG(ext1, 6) << 3) |
                          (REG(ext2, 0) << 6) | (REG(ext1, 0) << 9));
  if (tb_cflags(s->base.tb) & CF_PARALLEL) {
...
DISAS_INSN(move_to_ccr) { gen_move_to_sr(env, s, insn, true); }
...
DISAS_INSN(pulse) { /* Implemented as a NOP.  */ }

DISAS_INSN(illegal) { gen_exception(s, s->base.pc_next, EXCP_ILLEGAL); }
...
DISAS_INSN(tas) {
  ...
  if (mode == 0) {
    ...
    src1 = tcg_temp_new();
    tcg_gen_atomic_fetch_or_tl(src1, addr, tcg_constant_tl(0x80), IS_USER(s),
                               MO_SB);
...
DISAS_INSN(mull) {
  ...
  if (m68k_feature(s->env, M68K_FEATURE_M68K)) {
    ...
    if (sign) {
      ...
      tcg_gen_negsetcond_i32(TCG_COND_NE, QREG_CC_V, QREG_CC_V, QREG_CC_Z);
    } else {
      tcg_gen_mulu2_i32(QREG_CC_N, QREG_CC_V, src1, DREG(ext, 12));
      /* QREG_CC_V is -(QREG_CC_V != 0), use QREG_CC_C as 0 */
      tcg_gen_negsetcond_i32(TCG_COND_NE, QREG_CC_V, QREG_CC_V, QREG_CC_C);
    }
...
DISAS_INSN(cmpm) {
  ...
  src = gen_ea_mode(env, s, 3, REG(insn, 0), opsize, NULL_QREG, NULL, EA_LOADS,
                    IS_USER(s));
  /* Post-increment load (mode 3) from Ax.  */
  dst = gen_ea_mode(env, s, 3, REG(insn, 9), opsize, NULL_QREG, NULL, EA_LOADS,
                    IS_USER(s));
...
static inline void shift_reg(DisasContext *s, uint16_t insn, int opsize) {
  ...
  if (left) {
    ...
    if (opsize == OS_LONG) {
      ...
      tcg_gen_movcond_i32(TCG_COND_EQ, QREG_CC_C, s32, zero, zero, QREG_CC_C);
    }
    tcg_gen_andi_i32(QREG_CC_C, QREG_CC_C, 1);

    /* X = C, but only if the shift count was non-zero.  */
    tcg_gen_movcond_i32(TCG_COND_NE, QREG_CC_X, s32, QREG_CC_V, QREG_CC_C,
                        QREG_CC_X);
...
DISAS_INSN(shift8_im) { shift_im(s, insn, OS_BYTE); }

DISAS_INSN(shift16_im) { shift_im(s, insn, OS_WORD); }

DISAS_INSN(shift_im) { shift_im(s, insn, OS_LONG); }

DISAS_INSN(shift8_reg) { shift_reg(s, insn, OS_BYTE); }

DISAS_INSN(shift16_reg) { shift_reg(s, insn, OS_WORD); }

DISAS_INSN(shift_reg) { shift_reg(s, insn, OS_LONG); }
...
DISAS_INSN(rotate_reg) {
  ...
  if (insn & 8) {
    ...
    /* if shift == 0, clear C */
    tcg_gen_movcond_i32(TCG_COND_EQ, QREG_CC_C, t0, QREG_CC_V /* 0 */,
...
DISAS_INSN(moves) {
  ...
  case 3: /* Indirect postincrement.  */
    tcg_gen_addi_i32(
        AREG(insn, 0), addr,
        REG(insn, 0) == 7 && opsize == OS_BYTE ? 2 : opsize_bytes(opsize));
    break;
  case 4: /* Indirect predecrememnt.  */
    tcg_gen_mov_i32(AREG(insn, 0), addr);
    break;
...
DISAS_INSN(move_from_usp) {
  ...
  tcg_gen_ld_i32(AREG(insn, 0), tcg_env, offsetof(CPUM68KState, sp[M68K_USP]));
}
...
DISAS_INSN(move_to_usp) {
  ...
  tcg_gen_st_i32(AREG(insn, 0), tcg_env, offsetof(CPUM68KState, sp[M68K_USP]));
}
...
DISAS_INSN(wddata) { gen_exception(s, s->base.pc_next, EXCP_PRIVILEGE); }
...
DISAS_INSN(trap) { gen_exception(s, s->pc, EXCP_TRAP0 + (insn & 0xf)); }
...
static void gen_op_fmove_fcr(CPUM68KState *env, DisasContext *s, uint32_t insn,
                             uint32_t ext) {
  int mask = (ext >> 10) & 7;
  ...
  case 7: /* Immediate */
    if (REG(insn, 0) == 4) {
      if (is_write ||
          (mask != M68K_FPIAR && mask != M68K_FPSR && mask != M68K_FPCR)) {
        gen_exception(s, s->base.pc_next, EXCP_ILLEGAL);
        return;
      }
      tmp = tcg_constant_i32(read_im32(env, s));
...
static void gen_op_fmovem(CPUM68KState *env, DisasContext *s, uint32_t insn,
                          uint32_t ext) {
  int opsize;
  TCGv addr, tmp;
  int mode = (ext >> 11) & 0x3;
...
static void register_opcode(disas_proc proc, uint16_t opcode, uint16_t mask) {
  int i;
  int from;
  int to;
  ...
  if (opcode & ~mask) {
    fprintf(stderr, "qemu internal error: bogus opcode definition %04x/%04x\n",
            opcode, mask);
    abort();
  }
  /*
...
static void m68k_tr_init_disas_context(DisasContextBase *dcbase,
                                       CPUState *cpu) {
...
static void m68k_tr_insn_start(DisasContextBase *dcbase, CPUState *cpu) {
  ...
  if (dc->base.plugin_enabled) {
    TCGv cc_op =
        dc->cc_op == CC_OP_DYNAMIC ? QREG_CC_OP : tcg_constant_i32(dc->cc_op);
...
void m68k_translate_code(CPUState *cpu, TranslationBlock *tb, int *max_insns,
                         vaddr pc, void *host_pc) {
  DisasContext dc;
  translator_loop(cpu, tb, max_insns, pc, host_pc, &m68k_tr_ops, &dc.base);
}

static double floatx80_to_double(CPUM68KState *env, uint16_t high,
                                 uint64_t low) {
...
void m68k_cpu_dump_state(CPUState *cs, FILE *f, int flags) {
  ...
  for (i = 0; i < 8; i++) {
    qemu_fprintf(
        ...
        i, env->dregs[i], i, env->aregs[i], i, env->fregs[i].l.upper,
        env->fregs[i].l.lower,
        floatx80_to_double(env, env->fregs[i].l.upper, env->fregs[i].l.lower));
  }
  qemu_fprintf(f, "PC = %08x   ", env->pc);
  sr = env->sr | cpu_m68k_get_ccr(env);
  qemu_fprintf(f, "SR = %04x T:%x I:%x %c%c %c%c%c%c%c\n", sr,
               (sr & SR_T) >> SR_T_SHIFT, (sr & SR_I) >> SR_I_SHIFT,
               (sr & SR_S) ? 'S' : 'U', (sr & SR_M) ? '%' : 'I',
               (sr & CCF_X) ? 'X' : '-', (sr & CCF_N) ? 'N' : '-',
               (sr & CCF_Z) ? 'Z' : '-', (sr & CCF_V) ? 'V' : '-',
               (sr & CCF_C) ? 'C' : '-');
  qemu_fprintf(
      f, "FPSR = %08x %c%c%c%c ", env->fpsr,
      (env->fpsr & FPSR_CC_A) ? 'A' : '-', (env->fpsr & FPSR_CC_I) ? 'I' : '-',
      (env->fpsr & FPSR_CC_Z) ? 'Z' : '-', (env->fpsr & FPSR_CC_N) ? 'N' : '-');
  qemu_fprintf(f,
               "\n                                "
               "FPCR =     %04x ",
               env->fpcr);
  ...
  qemu_fprintf(f, "SSW %08x TCR %08x URP %08x SRP %08x\n", env->mmu.ssw,
               env->mmu.tcr, env->mmu.urp, env->mmu.srp);
