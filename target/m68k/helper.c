...
void m68k_switch_sp(CPUM68KState *env) {
  ...
  if (m68k_feature(env, M68K_FEATURE_M68K)) {
    ...
    new_sp =
        (env->sr & SR_S && env->cacr & M68K_CACR_EUSP) ? M68K_SSP : M68K_USP;
  }
  ...
  for (unsigned i = 0; i < M68K_ROOT_POINTER_ENTRIES; i++) {
    ...
    for (unsigned j = 0; j < M68K_ROOT_POINTER_ENTRIES; j++) {
      ...
      for (unsigned k = 0; k < tic_size; k++) {
        ...
        logical = (i << M68K_TTS_ROOT_SHIFT) | (j << M68K_TTS_POINTER_SHIFT) |
                  (k << tic_shift);
...
static int get_physical_address(CPUM68KState *env, hwaddr *physical, int *prot,
  ...
  for (i = 0; i < M68K_MAX_TTR; i++) {
    if (check_TTR(env->mmu.TTR(access_type, i), prot, address, access_type)) {
...
uint32_t HELPER(get_ccr)(CPUM68KState *env) { return cpu_m68k_get_ccr(env); }
...
void HELPER(reset)(CPUM68KState *env) { /* FIXME: reset all except CPU */ }
#endif /* !CONFIG_USER_ONLY */
